...
def extract_input(config_path: str):
    ...
    Extract input, output and air scattering file paths, 
    ...
    air_scatter_path = config["file_paths"]["air_scatter_path"]
    print("The path to the air scatter image is :", air_scatter_path, sep = '\n', end = '\n\n')
    
    air_scatter_multiple = config["file_paths"]["air_scatter_multiple"]
    print("The value to multiply the air scatter intensity, to match the acquisition frequency of the data, is :", air_scatter_multiple, sep = '\n', end = '\n\n')
    
    return experiment_numbers, input_path, output_path, air_scatter_path, air_scatter_multiple
...
def subtract_tiff_images(experiment_number: str, air_scatter_filepath: str, air_scatter_multiple: int, input_filepath: str, output_filepath: str, v_max: int):
    '''Subtract a tiff image (such as an air scattering image) 
    from each of the tiff images contained in the input folder
    and save the subtracted tiff images to the output folder.
    
    :param experiment_number: input experiment number.
    :param air_scatter_filepath: path of the tiff image to subtract (such as an air scattering image).
    :param air_scatter_multiple: value to multiply the air scatter intensity, to match the acquisition frequency of the data.
    :param input_filepath: input path to the series of tiff images.
    ...
    image_array = np.zeros(shape=np.shape(image_example), dtype='int32')
    air_scatter_image_array = np.zeros(shape=np.shape(image_example), dtype='int32')
    ...
    air_scatter_image_array = np.array(io.imread(air_scatter_filepath))
    subtract_image_array = air_scatter_multiple * air_scatter_image_array
    ...
    print(f"The AIR SCATTER image, along with the BEFORE / AFTER subtraction images for the final image in the series, are shown below...", sep = '\n', end = '\n\n')
...
def multiple_subtract_tiff_images(experiment_numbers: List[int], air_scatter_filepath: str, air_scatter_multiple: int, input_path: str, output_path: str, v_max: int):
    '''Create input and output file paths for a list of experiments. 
    Subtract a tiff image (such as an air scattering image) 
    from each of the tiff images contained in the input folder
    and save the subtracted tiff images to the output folder.
    ...
    for experiment_number in experiment_numbers:
        ...
        subtract_tiff_images(experiment_number, air_scatter_filepath, air_scatter_multiple, input_filepath, output_filepath,  v_max)